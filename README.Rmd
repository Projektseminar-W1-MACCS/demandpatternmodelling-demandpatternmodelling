...
The first sub-function and output parameter which is set by the production environment is the demand function. Here, for the first time it becomes possible for the modeller to have an influence on the output, without changing the input parameters, by changing how the demand vector is build.
...
The procedure for the construction of the resource consumption matrix can be seen below. 
<br>
...
BASE is the first resource in the resource consumption matrix, that is used in every product and therefore is built seperatly. 
...
There are two correlations, one for the big resources and one for the small resources, meaning that the big resources correlate among themselves with approximately COR1 and the small resources with COR2 respectively. 
The correlations are drawn form a uniform distribution with boundaries set by the modeller. 
