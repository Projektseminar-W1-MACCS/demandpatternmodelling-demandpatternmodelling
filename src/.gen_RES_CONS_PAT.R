...
.gen_RES_CONS_PAT_DISP2 <- function(FIRM) {
  ...
  repeat    {
    ...
    RES_CONS_PATpre = matrix(rnorm(FIRM$PRODUCTION_ENVIRONMENT$NUMB_PRO*(FIRM$PRODUCTION_ENVIRONMENT$NUMB_RES-1),mean=0,sd=1), 
                             FIRM$PRODUCTION_ENVIRONMENT$NUMB_PRO, (FIRM$PRODUCTION_ENVIRONMENT$NUMB_RES-1))                            #random pre matrix, as Baseline
...
.gen_RES_CONS_PAT_DISP3 <- function(FIRM) {
  ...
  repeat    {
    ...
    for (i in 2:(DISP1)) #unitsize+1
    {
      RES_CONS_PAT[,i] <- runif(BASE,-0.2,0.8);
    }
